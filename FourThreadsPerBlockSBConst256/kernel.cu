...
//Even Round Function
__device__ void FE_d(uint8* textR, const uint8* sMemKeys, uint8* sMemText, unsigned int offset16)
{
    ...
    textR[0] = SB3_dev[textR[0]];
    textR[1] = SB4_dev[textR[1]];
    textR[2] = SB1_dev[textR[2]];
    textR[3] = SB2_dev[textR[3]];
...
//Odd Round Function
__device__ void F0_d(uint8* textR, const uint8* sMemKeys, uint8* sMemText, unsigned int offset16)
{
    //XOR with the round key
    #pragma unroll
    for(int i=0; i < 4; i++){
    ...
    textR[0] = SB1_dev[textR[0]];  
    textR[1] = SB2_dev[textR[1]];  
    textR[2] = SB3_dev[textR[2]];  
    textR[3] = SB4_dev[textR[3]];
...
__global__ void Encrypt(uint8* textG, unsigned long int textSize, uint8* ek)
{
    ...
    //SB's are kept in constant memory.
    ...
    */
    ...
    {
        F0_d(textR, sMemKeys, sMemTextBlockAddr, offset16);//ek1... 
        FE_d(textR, sMemKeys + 16, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 32, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 48, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 64, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 80, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 96, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 112, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 128, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 144, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 160, sMemTextBlockAddr, offset16);//...ek11
    ...
    else if(keySize == 24)//192-bit keys
    {
        ...
        F0_d(textR, sMemKeys + 128, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 144, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 160, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 176, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 192, sMemTextBlockAddr, offset16);//...ek13
    ...
    else//256-bit keys
    {
        ...
        F0_d(textR, sMemKeys + 160, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 176, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 192, sMemTextBlockAddr, offset16);
        FE_d(textR, sMemKeys + 208, sMemTextBlockAddr, offset16);
        F0_d(textR, sMemKeys + 224, sMemTextBlockAddr, offset16);//...ek15
