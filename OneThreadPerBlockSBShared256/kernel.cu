...
__global__ void Encrypt(uint8* plainText, unsigned long textSize, uint8* ek, uint8* SB_gmem)
{
	...
	//Load encryption round keys to shared memory.
	if (tid < 256) {
		keySmem[tid] = ek[tid];
	}
	//rest of the bytes(272 bytes) are loaded by first 16 threads.
	if (tid < 16) {
		keySmem[256 + tid] = ek[256 + tid];
	}
	...
	{
		F0_d(plainTextR, &keySmem[0], SB1, SB2, SB3, SB4);//ek1...   
		FE_d(plainTextR, &keySmem[16], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[32], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[48], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[64], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[80], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[96], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[112], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[128], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[144], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[160], SB1, SB2, SB3, SB4);//...ek11

		#pragma unroll
		for (int i = 0; i < 16; i++) {
			plainTextR[i] = plainTextR[i] ^ keySmem[176 + i];//ek12
		}

		plainTextR[0] = SB3[plainTextR[0]];
		plainTextR[1] = SB4[plainTextR[1]];
		plainTextR[2] = SB1[plainTextR[2]];
		plainTextR[3] = SB2[plainTextR[3]];
		plainTextR[4] = SB3[plainTextR[4]];
		plainTextR[5] = SB4[plainTextR[5]];
		plainTextR[6] = SB1[plainTextR[6]];
		plainTextR[7] = SB2[plainTextR[7]];
		plainTextR[8] = SB3[plainTextR[8]];
		plainTextR[9] = SB4[plainTextR[9]];
		plainTextR[10] = SB1[plainTextR[10]];
		plainTextR[11] = SB2[plainTextR[11]];
		plainTextR[12] = SB3[plainTextR[12]];
		plainTextR[13] = SB4[plainTextR[13]];
		plainTextR[14] = SB1[plainTextR[14]];
		plainTextR[15] = SB2[plainTextR[15]];

		#pragma unroll
		for (int i = 0; i < 16; i++) {
			plainTextR[i] = plainTextR[i] ^ keySmem[192 + i];//ek13
		}
	...
	else if (keySize == 24)//192-bit keys
	{
		...
		F0_d(plainTextR, &keySmem[128], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[144], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[160], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[176], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[192], SB1, SB2, SB3, SB4);//ek13

		#pragma unroll
		for (int i = 0; i < 16; i++) {
			plainTextR[i] = plainTextR[i] ^ keySmem[208 + i];//ek14
		}
		...
		#pragma unroll
		for (int i = 0; i < 16; i++) {
			plainTextR[i] = plainTextR[i] ^ keySmem[224 + i];//ek15
		}
	...
	else//256-bit keys
	{
		...
		F0_d(plainTextR, &keySmem[160], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[176], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[192], SB1, SB2, SB3, SB4);
		FE_d(plainTextR, &keySmem[208], SB1, SB2, SB3, SB4);
		F0_d(plainTextR, &keySmem[224], SB1, SB2, SB3, SB4);//ek15

		#pragma unroll
		for (int i = 0; i < 16; i++) {
			plainTextR[i] = plainTextR[i] ^ keySmem[240 + i];//ek16
		}
		...
		#pragma unroll
		for (int i = 0; i < 16; i++) {
			plainTextR[i] = plainTextR[i] ^ keySmem[256 + i];//ek17
		}
